...
class DistantGalaxy(object):
    ...
    def get_bandlumin(self, evolution=True, universe_rad=450000):
        ...
        if self.bandlumin == False:
            ...
            a, b, c = bandlumin_coeffs[self.species]
            if evolution:
                d = - (self.spherical[2] / universe_rad) * 100
            else:
                d = 0
            surge_lumin = lambda x: a * ((x + d) * 10**-9)**b * np.exp(c * ((x + d) * 10**-9))
            
            bands = np.array([440, 500, 700])
            self.bandlumin = surge_lumin(bands) * np.random.normal(1, 0.05, 3)
