...
position = np.array([46, 90, 1000])
position = spherical_to_cartesian(46, 90, 1000)
radius = 10
...
Gposition = [45, 90, 1000]
species = 'SBa'
galax = Galaxy(species, Gposition)

spiralNeb = Nebula(species, Gposition, galax.radius, rotation=galax.rotation)
...
for c, colour in enumerate(colourmap):
    ...
    # density = density + 30 * (density - scipy.ndimage.gaussian_filter(density, 1))
